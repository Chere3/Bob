        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (warns.length < 10) {
                ...
                for (let i = 0; i < ws.length; i++) {
                    emds.push(new discord_js_1.MessageEmbed().setDescription(`${constructWarns(ws[i], i).slice(9)}`).setAuthor(`Warns de ${member.user.tag}`, yield base.avatar(member.id)).setColor(`PURPLE`).setFooter(`Página ${i + 1} de ${ws.length} · ${warns.length} warns.`));
                }
                var p = 0;
