...
export default class evalCommand extends revisionManager {
    ...
    async run(b: run) {
        ...
        if (b.flags.includes(`async`)) evaled = eval(`(async () => {${b.args.join(" ")}})()`)
        
        b.send({embeds: [embed.setDescription(`\`\`\`typescript\n${util.inspect(evaled, true, 0)}\`\`\``)]}).catch(async () => {
            const arrays = await new utilManager().divideTextIntoArrays(util.inspect(evaled, true, 0), 4000); const embeds = [] as MessageEmbed[]
            const m1 = await b.send({embeds: [embed.setDescription(`\`\`\`typescript\n${arrays[0]}\`\`\``)], components: [b.defaultPaginator]});



            for (let i = 0; i < arrays.length; i++) {
                embeds.push(new MessageEmbed({color: `PURPLE`, description: `\`\`\`typescript\n${arrays[i]}\`\`\``}))
            }
            
            await new messageCollect()
            .setAuthor(b.author)
            .setMessage(m1)
            .setEmbeds(embeds)
            .start()
