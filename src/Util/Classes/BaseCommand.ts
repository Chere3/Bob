  ...
  canRun(msg, isDev: boolean) {
    ...
    if (this.dev === true && !owners.includes(msg.author.id))
      return msg.channel.send(
        e.setDescription(
          `❌ | Lo siento, pero este comando esta creado solo para owners.`
        )
      );
    if (this.guildOnly === true && !msg.guild)
      return msg.channel.send(
        e.setDescription(
          `❌ | Este comando solo esta permitido dentro de servidores.`
        )
      );
    if (!this.status && !owners.includes(msg.author.id))
      return msg.channel.send(
        e.setDescription(`❌ | Comando bajo mantenimiento o desabilitado.`)
      );
    if (this.checkCooldown(msg) && !owners.includes(msg.author.id)) {
      ...
      return msg.channel.send(
        e.setDescription(
          `❌ | Hey, tranquilo, puedes ejecutar este comando de nuevo en ${timeLeft.toFixed(
            1
          )} segundos.`
        )
      );
    ...
    if (
      ...
      !this.botPermissions.some((x) =>
        msg.guild.me.hasPermission(x as PermissionResolvable)
      )
    ) {
      return msg.channel.send(
        e.setDescription(
          `❌ | No tengo los siguientes permisos para el comando: \`${this.botPermissions
            .map((x) => this.parsePermission(x))
            .join(", ")}\``
        )
      );
    }
    if (
      msg.guild &&
      this.memberPermissions[0] &&
      !this.memberPermissions.some((x) =>
        msg.member.hasPermission(x as PermissionResolvable)
      ) &&
      !isDev
    ) {
      return msg.channel.send(
        e.setDescription(
          `❌ | No tienes los suficientes permisos para correr el comando: \`${this.memberPermissions
            .map((x) => this.parsePermission(x))
            .join(", ")}\``
        )
      );
